            ...
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                ...
                <div>
                  ...
                  <div className="bg-bg-main p-3 md:p-4 rounded border border-border-base shadow-sm space-y-3 md:space-y-4 text-xs md:text-sm">
                    <div className="grid grid-cols-2 gap-3 md:gap-4">
                      <DetailItem
                        label="Credito"
                        value={`${row.credito || 0} fiorini`}
                      />
                      <DetailItem
                        label="Credito ai Monti"
                        value={`${row.credito_m || 0} fiorini`}
                      />
                      <DetailItem
                        label="Fortune"
                        value={`${row.fortune || 0} fiorini`}
                      />
                      <DetailItem
                        label="Deduzioni"
                        value={`${row.deduzioni || 0} fiorini`}
                      />
                      <div>
                        <span className="text-[10px] md:text-xs text-text-accent uppercase block">
                          Imponibile Totale
                        </span>
                        <span className="font-bold text-item-selected text-base md:text-lg">
                          {row.imponibile || 0} fiorini
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="pt-2 border-t border-dashed border-border-base space-y-2">
                      ...
                      <DetailRow
                        icon={Flag}
                        label="Immigrazione"
                        value={row.immigrazione}
                      />
                      <DetailRow
                        icon={Hammer}
                        label="Rapporto Mestiere"
                        value={row.rapporto_mestiere}
                      />
                    </div>

                    <div className="pt-2 border-t border-dashed border-border-base">
                      <span className="text-[10px] md:text-xs text-text-accent uppercase block mb-1">
                        Particolarità Fuoco
                      </span>
                      <p className="italic text-text-main bg-bg-sidebar p-2 rounded border border-border-base">
                        {row.particolarita_fuoco ||
                          "Nessuna particolarità registrata."}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  {loadingParenti ? (
                    <div className="text-sm text-text-accent italic">
                      Caricamento...
                    </div>
                      ...
                      <table className="min-w-full divide-y divide-border-base">
                        ...
                        <tbody className="bg-bg-main divide-y divide-border-base">
                          {parentiData.map((parente, idx) => (
                            <tr
                              key={idx}
                              className="text-xs md:text-sm text-text-main"
                            >
                              <td className="px-2 py-2 font-medium">
                                {parente.parentela_desc || "Membro"}
                                {parente.sesso && (
                                  <span className="text-[10px] text-text-accent ml-1">
                                    ({parente.sesso})
                                  </span>
                                )}
                              </td>
                              <td className="px-2 py-2">
                                {parente.eta ? parente.eta : "-"}
                              </td>
                              <td className="px-2 py-2">
                                <div className="flex flex-col">
                                  <span>{parente.stato_civile}</span>
                                  <span className="text-[10px] italic text-text-accent">
                                    {parente.particolarita}
                                  </span>
                                </div>
                              </td>
                            </tr>
