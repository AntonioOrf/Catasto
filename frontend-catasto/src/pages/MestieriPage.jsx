  ...
  return (
    <div className="min-h-screen flex flex-col bg-bg-main text-text-main font-serif">
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full p-6 md:p-10">
        ...
        {loading && <p className="text-text-muted">Caricamento in corso...</p>}
        {error && <p className="text-red-500">{error}</p>}
        
        {!loading && !error && (
          <div className="bg-bg-dark border border-border-color rounded-lg overflow-hidden shadow-lg">
            <table className="w-full text-left border-collapse">
              <thead className="bg-bg-hover text-text-dim border-b border-border-color text-sm uppercase">
                <tr>
              ...
              <tbody className="divide-y divide-border-color text-sm">
                {mestieri.map((m) => (
                  <tr key={m.id || m.ID_Mestiere || m.Mestiere} className="hover:bg-bg-hover/50 transition-colors">
